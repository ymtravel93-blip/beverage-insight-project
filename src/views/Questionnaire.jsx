...
export default function Questionnaire({ onComplete }) {
  ...
  return (
    <div className="w-full bg-[#F5F3EF] min-h-screen text-[#121212]">
      <div className="max-w-3xl mx-auto px-6 py-8 flex flex-col min-h-[calc(100vh-100px)] justify-between">
        <div>
          ...
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="mt-4"
            >
              <span className="text-[10px] uppercase tracking-[0.25em] text-[#B38B59] font-semibold block mb-2">
                {currentQ.section}
              </span>
              ...
              <p className="text-xs sm:text-sm text-[#55524D] font-light mb-8">
                {currentQ.subtitle}
              </p>
              ...
              {currentQ.type === 'dual_select' && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                  <div>
                    <label className="text-[10px] text-[#55524D] uppercase tracking-wider block mb-2 font-medium">Location</label>
                    <select
                      value={answers[currentQ.id]?.city || ''}
                      onChange={(e) => handleDualSelect('city', e.target.value)}
                      className="w-full p-4 bg-[#FFFFFF] border border-[#D9D5CC] text-[#121212] text-sm rounded-sm focus:outline-none focus:border-[#B38B59]"
                    >
                  ...
                  <div>
                    <label className="text-[10px] text-[#55524D] uppercase tracking-wider block mb-2 font-medium">Age Group</label>
                    <select
              ...
              {/* SCALE 1-5 */}
              {currentQ.type === 'scale' && (
                <div className="py-8 px-6 bg-[#FFFFFF] border border-[#D9D5CC] rounded-sm">
                      ...
                      <button
                        ...
                        className={`w-12 h-12 rounded-full border text-sm font-medium transition-all ${
                          currentAnswers[0] === num.toString()
                            ? 'bg-[#B38B59] text-[#FFFFFF] border-[#B38B59] shadow'
                            : 'border-[#D9D5CC] bg-[#F5F3EF] text-[#121212] hover:border-[#B38B59]'
                        }`}
                      >
                        {num}
                      </button>
                  ...
                  <div className="flex justify-between max-w-md mx-auto text-xs text-[#55524D] font-light">
        ...
        {/* Navigation Footer */}
        <div className="pt-8 pb-4 flex justify-between items-center border-t border-[#D9D5CC] mt-8">
          <button
            onClick={handleBack}
            disabled={currentIndex === 0 || isSubmitting}
            className={`text-xs uppercase tracking-widest font-medium ${
              currentIndex === 0 ? 'text-[#D9D5CC] cursor-not-allowed' : 'text-[#55524D] hover:text-[#121212]'
            }`}
          >
            ← Back
          </button>
          ...
          <button
            ...
            className={`px-8 py-3.5 text-xs uppercase tracking-[0.2em] font-medium transition-all ${
              isValid && !isSubmitting
                ? 'bg-[#B38B59] text-[#FFFFFF] hover:bg-[#9B7547] cursor-pointer shadow-md'
                : 'bg-[#D9D5CC] text-[#888580] cursor-not-allowed'
            }`}
          >
